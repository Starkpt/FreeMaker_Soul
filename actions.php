...
switch ($act) {
    ...
    case 'del_user':
        ...
        if ($stmt === 0) {
            ...
            $conn->rollback();
            header('Location: adm_configs.php?msg=del_user_error');
            exit;
        } else {
            $_SESSION['info_msg'] = 'Utilizador eliminado com sucesso!';
            $conn->commit();
            header('Location: adm_configs.php?msg=del_user_success');
        }
        break;
        ...
        if ($_FILES['foto']['size'] !== 0) {
            ...
            if ($row['foto'] && file_exists('imgs/users/profile_pics/' . $row['foto'])) {
                ...
                if (file_exists('imgs/users/profile_pics/' . $row['foto'])) {
                    ...
                    header('Location: adm_configs.php?msg=edit_user_error');
                    exit;
                }
            }
            ...
            if ($stmt->affected_rows === 0) {
                ...
                header('Location: adm_configs.php?msg=edit_user_success');
            }
        } else {
            // Editar sem foto
            $stmt = $conn->prepare('UPDATE utilizadores SET nome = ?, morada = ?, cod_postal = ?, telefone = ?,
    ...
    case 'del_prod':
        ...
        if (!$tudoOk) {
            ...
            $conn->rollback();
            header('Location: adm_configs.php?msg=del_prod_error');
            exit;
        } else {
            ...
            if (!$tudoOk) {
                ...
                $conn->commit();
                header('Location: adm_configs.php?msg=del_prod_success');
            }
        }
